...
- `padrao`: `erro`, `nota`, `tipo` (coluna "TIPO DE ERRO", renomeada de "CONFERÊNCIA" — nem toda planilha `padrao` foi renomeada ainda, então `tipo` fica vazio até a planilha específica ser atualizada), `gravidade`, `consequencia`, `visibilidade`.
...
Cada campo do item (`tipo`, `classificador`, `gravidade`, `consequencia`, `visibilidade`) só aparece como tag em `render()` (`js/app.js`) se estiver preenchido — ao adicionar um novo campo de coluna, siga esse mesmo padrão condicional em vez de assumir que o valor sempre existe.
